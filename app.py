...
def get_pipeline(model_name):
    ...
    if model_name == "epicrealism":
        ...
        pipeline = QwenImageEditPipeline.from_pretrained(
            "Qwen/Qwen-Image-Edit", scheduler=scheduler, torch_dtype=torch.bfloat16, cache_dir=CACHE_DIR
        ).to("cuda")
        pipeline.load_lora_weights(
            "lightx2v/Qwen-Image-Lightning", weight_name="Qwen-Image-Edit-Lightning-8steps-V1.0.safetensors", cache_dir=CACHE_DIR
        )
    else:
...
with gr.Blocks() as interface:
    with gr.Tabs():
        ...
        with gr.TabItem("qwen-image-edit + lightning"):
            ...
            generate_button_edit.click(fn=generate_qwen_edit, inputs=[input_image_edit, prompt_edit, negative_prompt_edit, sampling_steps_edit], outputs=[output_edit])
